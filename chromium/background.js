    ...
    chrome.storage.local.get(Object.keys(defaultSettings), (result) => {
        ...
        Object.keys(defaultSettings).forEach(key => {
            if (result[key] === undefined) {
                settingsToSave[key] = defaultSettings[key];
            }
        });
...
async function executeContentScript(url, tabId) {
    try {
        const { contentScripts } = await chrome.storage.local.get('contentScripts');
    ...
    switch (message.action) {
        case 'getLocalStorageValue':
            if (message.requestType) {
                chrome.storage.local.get(message.requestType, (result) => {
                    sendResponse({ value: result[message.requestType] });
                });
            } else {
